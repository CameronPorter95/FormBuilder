...
private let serverDir = ProcessInfo.processInfo.environment["SERVER_DIR"]

final class StubsMiddleware: Middleware {
    public func respond(to request: Request, chainingTo next: Responder) throws -> Future<Response> {
        guard let projectDir = serverDir else {
            throw RouteError(message: "SERVER_DIR environment variable not set")
        }
